...
export const getAutoFillResults = async (currentInput) => {
    ...
    const res = await fetch(`https://api.edamam.com/auto-complete?app_id=4fd7bfa8&app_key=%2090a867d4fdf8ecfdc3258b489ab7fbbc%09&q=${transformedInput}&limit=6`)
    const arrayOfFoods = await res.json()
    return arrayOfFoods
}

export const getFoodData = async (food) => {
    const res = await fetch(`https://api.edamam.com/api/food-database/v2/parser?app_id=4fd7bfa8&app_key=90a867d4fdf8ecfdc3258b489ab7fbbc&ingr=${food}&nutrition-type=cooking`)
    ...
    const cleanedData = arrayOfFoods.map((item) => {
        ...
        return {
            ...
            label: foodObj.label,
            nutrients: {
                calories: foodObj.nutrients.ENERC_KCAL ? Number(foodObj.nutrients.ENERC_KCAL.toFixed(2)) : 0,
                protein: foodObj.nutrients.PROCNT ? Number(foodObj.nutrients.PROCNT.toFixed(2)) : 0,
                fat: foodObj.nutrients.FAT ? Number(foodObj.nutrients.FAT.toFixed(2)) : 0,
                carbs: foodObj.nutrients.CHOCDF ? Number(foodObj.nutrients.CHOCDF.toFixed(2)) : 0,
                fibre: foodObj.nutrients.FIBTG ? Number(foodObj.nutrients.FIBTG.toFixed(2)) : 0
            },
            image: foodObj.image || ''
        }
    })
...
export const getActivitiesList = async () => {
    const res = await fetch('https://api.api-ninjas.com/v1/caloriesburnedactivities', {
        method: 'GET',
        headers: {
...
export const getActivityDetails = async (activity, weight) => {
    ...
    if (weight) {
        const res = await fetch(`https://api.api-ninjas.com/v1/caloriesburned?activity=${activity}&weight=${weight}`, {
            method: 'GET',
            headers: {
                "X-Api-Key": "M3E6uLa4vo3d5xnU6I5/YQ==P8C5PsirDGOxDnro"
            }
        ...
        const res = await fetch(`https://api.api-ninjas.com/v1/caloriesburned?activity=${activity}`, {
            method: 'GET',
            headers: {
                "X-Api-Key": "M3E6uLa4vo3d5xnU6I5/YQ==P8C5PsirDGOxDnro"
            }
