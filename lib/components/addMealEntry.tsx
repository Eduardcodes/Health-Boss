...
export default function AddMealEntry({ingredient}:{ingredient: FoodData}) {
  ...
  return (
    <div>
      ...
      <div>
          <p>Calories: {(ingredient.nutrients.calories*(amount/100)).toFixed(2)}</p>
          <p>Protein: {(ingredient.nutrients.protein*(amount/100)).toFixed(2)}</p>
          <p>Carbohydrates: {(ingredient.nutrients.carbs*(amount/100)).toFixed(2)}</p>
