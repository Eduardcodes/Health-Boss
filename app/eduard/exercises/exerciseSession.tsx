...
export default function ExerciseSession(
  ...
  {setModalOpen, setAddSessionBox, setDisplayedSessions, selectedActivities, setSelectedActivities}:
    ...
    for(const activityName in activityNamesAndDurations) {
      for(let i=0; i<selectedActivities.length; i++) {
        if(activityName === selectedActivities[i].activity) {
          ...
          calsBurnedThisSesssion += selectedActivities[i].caloriesPerHour*(duration/60)
          activityList.push({
